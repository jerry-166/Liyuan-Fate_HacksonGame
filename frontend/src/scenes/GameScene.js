...
export class GameScene extends Phaser.Scene {
  ...
  _toggleEditMode() {
    ...
    if (this._editMode) {
      ...
      console.log('[Editor] ✅ 已进入编辑模式 — 点击设置碰撞, 拖拽NPC移动位置');
    } else {
  ...
  _refreshEditHUD() {
    ...
    const mode = this._draggedNPC ? '🔵 拖拽NPC中...' : '🟢 左键:切换碰撞 | 拖拽:NPC | S:保存 | C:清除全部';
    this._editHUDText.setText(`[碰撞编辑模式]\n${mode}\n当前碰撞格数: ${count}`);
  }
  ...
  _getPointerTile(ptr) {
    ...
    return {
      col: Math.floor(worldX / gridPx),
      row: Math.floor(worldY / gridPx),
    };
  }
  ...
  /** 保存碰撞配置到 localStorage + 控制台导出 JSON */
  ...
  _showControlsHint() {
    ...
    const hint = this.add.text(width / 2, height - 50, [
      '🎮 [WASD/方向键] 移动  |  [E] 碰撞编辑器  |  [R] 硬重置(清缓存)  |  [C] 清除碰撞',
      '提示: 按E进入编辑模式可设置碰撞区域和拖拽NPC位置',
    ].join('\n'), {
